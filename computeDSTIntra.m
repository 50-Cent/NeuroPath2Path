function [dsTNeuron,dstVecforPair,prL] = computeDSTIntra(dataA,dataB)
...
for k = 1:noPath1
    ...
    for m = 1:noPath2
        ...
        if length(tort11) >= length(tort2)
            tort2 = [zeros(length(tort11)-length(tort2),1);tort2] ;           
            dstTort = norm((tort1-tort2).*bOrder1,2);            
        elseif length(tort2) > length(tort1)           
            tort11 = [zeros(length(tort2)-length(tort11),1);tort11];
        ...
        if length(bifur11) >= length(bifur2)
            bifur2 = [zeros(length(bifur11)-length(bifur2),1);bifur2];
            dstBifur = norm((bifur11-bifur2).*bOrder1,2);
        elseif length(bifur2) > length(bifur11)          
            bifur11 = [zeros(length(bifur2)-length(bifur11),1);bifur11];
        ...
        if length(caul11) >= length(caul2)
            caul2 = [zeros(length(caul11)-length(caul2),1);caul2];
            dstCaul = norm((caul11-caul2).*bOrder1,2);
        elseif length(caul2) > length(caul11)           
            caul11 = [zeros(length(caul2)-length(caul11),1);caul11];
        ...
        if length(conc11) >= length(conc2)
            conc2 = [zeros(length(conc11)-length(conc2),1);conc2];
            dstConc = norm((conc11-conc2).*bOrder1,2);
        elseif length(conc2) > length(conc11)           
            conc11 = [zeros(length(conc2)-length(conc11),1);conc11];
        ...
        if length(segL11) >= length(segL2)
            segL2 = [zeros(length(segL11)-length(segL2),1);segL2];
            dstSegL = norm((segL11-segL2).*bOrder1,2)/sqrt(length(segL1));
        elseif length(segL2) > length(segL11)            
            segL11 = [zeros(length(segL2)-length(segL11),1);segL11];
        ...
        if length(compt11) >= length(compt2)
            compt2 = [zeros(length(compt11)-length(compt2),1);compt2];
            dstCompt = norm((compt11-compt2).*bOrder1,2);
        elseif length(compt2) > length(compt11)           
            compt11 = [zeros(length(compt2)-length(compt11),1);compt11];
        ...
        dstCell{k,m} = dstVec;        
        distanceMat(k,m) = [0.0785 0.0807 0.1736 0.1956 0.2600 0.2076]*dstVec;          
    end
end
